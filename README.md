...
Далее в личном кабинете в настройках аккаунта мы указываем адрес для WebHook`a – в моём случае это http://субдомен.домен.ru/get_messages.
В Вашем случае, если Вы будете использовать отдельный домен, это будет выглядеть примерно так: http://домен.ru/get_messages.
...
module.exports = {
    ...
    apiUrl: "https://eu14.chat-api.com/instance255316/",
...
```
Подробнее:  prequest позволит совершать запросы к API, config подгрузит наши данные с другого файла, в переменные token и apiUrl сразу поместим данные из конфига (чтобы проще было к ним обращаться).
...
В моём случае пусть это будет get_messages (т.е. вебхук будет отправлять запросы по адресу http://yoursite.ru/get_messages)
...
Теперь запишем информацию о полученном сообщении в переменные:
```javascript
app.post('/get_messages', async function (req, res) {
    const data = req.body;
    for (var i in data.messages) {
